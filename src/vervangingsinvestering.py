...
def load_data():
    available_files = {
        "Excel": "../data/excel/Integrale kosten Wegen v04.0 LIVE.xlsx",
        "CSV": "../data/csv/sample_data.csv"
    }
    file_choice = st.sidebar.selectbox("Select data source", list(available_files.keys()))
    file_path = available_files[file_choice]
    try:
        df = load_data_file(file_path)
        df = df[:-1].copy()  # If needed, drop last row
